...
public class salaryRepEmp extends JFrame 
{
  String[] colhead={"BASIC","DA","HRA","ALLOWANCE","GPF","GIP","GROSS","DEDUCTIONS","NET SALARY"}; 
 ...
 public salaryRepEmp()
 {
		...
		String mon[] = {"--select--","JAN" , "FEB" , "MAR" , "APR", "MAY" , "JUN" , "JUL" , "AUGUST", "SEPTEMBER" , "OCTOBER" , "NOVEMBER" , "DECEMBER" };
        cbMth = new JComboBox(mon);
		cbMth.setBounds(350,90,100,30);
       	//cn.add(lblEmpId);
		cn.add(lblMth);
		cn.add(cbMth);
		//cn.add(txtEmpId);
        ...
        dispall=new JButton("SHOW SALARY DETAILS");
...
public class dispallListener implements ActionListener
{
           ...
           try
    {
		...
		int i=0;
        rs=statement.executeQuery("select * from salary where eid='"+loginform.usr+"'"); 
        while(rs.next())
        {
            data[i][0]=rs.getString("BASIC");
            data[i][1]=rs.getString("DA");
			 data[i][2]=rs.getString("HRA");
            data[i][3]=rs.getString("ALLOWANCE");
			 data[i][4]=rs.getString("GPF");
			  data[i][5]=rs.getString("GIP");
			  data[i][6]=rs.getString("TOTAL");
			    data[i][7]=rs.getString("totded");
				 data[i][8]=rs.getString("netpay");
...
public class setupListener implements ActionListener
{
 ...
 {
         salaryRepEmp s=new  salaryRepEmp();
 }
